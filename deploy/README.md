...
> **먼저 도커가 도는지만 보고 싶으시면** — 검은 화면 없이 DSM 화면만으로
> 확인하는 짧은 길을 따로 적어두었습니다: **[container-manager.md](container-manager.md)**
> (20분, 도메인·인증서 없이). 잘 돌면 여기로 돌아오세요.

---

> **검은 화면(SSH)을 쓰고 싶지 않으시면** — DSM 화면에서 눌러서만 하는 길을
> 따로 적어두었습니다: **[nas-ui.md](nas-ui.md)**.
> 명령어는 딱 한 줄(유치원 자리 만들기)만 나오고, 그것도 붙여 넣기입니다.

## 언제 공유기를 건드리나 — 전체 순서

공유기 포트포워딩을 바꾸는 순간 **지금 돌고 있는 홈페이지가 맥미니에서 NAS 로
넘어갑니다.** 그러니 NAS 쪽이 완전히 준비되고 확인까지 끝난 다음에 건드립니다.

```
1단계  NAS 에 두 개를 띄운다            공유기 안 건드림
2단계  문지기(역방향 프록시)를 세운다     공유기 안 건드림
3단계  내 PC 에서 전체 길을 확인한다      공유기 안 건드림   ← 여기까지가 핵심
4단계  공유기를 한 번에 바꾼다
5단계  인증서를 받는다
6단계  밖에서 확인한다
```

3단계까지 끝나면 **홈페이지가 NAS 에서 이미 손님을 맞을 준비를 마친 상태**입니다.
그때 4단계에서 포트를 넘기면 끊김 없이 갈아탑니다.

### 1단계 — NAS 에 두 개를 띄운다

| | 어떻게 | 확인 |
|---|---|---|
| 손잡고 마중 | 이미지를 올려 컨테이너로 ([container-manager.md](container-manager.md))<br>또는 저장소를 받아 `sh deploy/nas-up.sh` | `curl http://127.0.0.1:8765/health` |
| 홈페이지 | `cd /volume1/docker/aurabus-site && docker compose up -d` | `curl http://127.0.0.1:8080/` |

확인은 **NAS 안에서** 합니다 (SSH 로 들어가서). 두 컨테이너 모두 `127.0.0.1` 에만
문을 열어 두어서, 다른 PC 에서 `192.168.100.10:8765` 로는 닿지 않는 것이 정상입니다.

```bash
ssh -p 9292 관리자아이디@192.168.100.10
curl -s -o /dev/null -w "마중  %{http_code}\n" http://127.0.0.1:8765/health
curl -s -o /dev/null -w "홈피  %{http_code}\n" http://127.0.0.1:8080/
```
...
7장대로 역방향 프록시 규칙 **세 개**를 만듭니다.
인증서는 아직 없으니 DSM 기본 인증서가 쓰입니다 — 브라우저가 경고를 내지만
**지금은 그게 정상**입니다. 길이 제대로 갈리는지만 보는 단계입니다.

### 3단계 — 내 PC 에서 전체 길을 확인한다 ← 가장 중요

도메인은 아직 학교 IP 를 가리키고, 그 IP 는 맥미니로 가고 있습니다. 그래서
**내 PC 만 잠깐 NAS 를 보도록** 속여서 확인합니다.

메모장을 **관리자 권한으로** 열어 이 파일을 엽니다.
...
저장한 뒤 브라우저에서 열어 봅니다 (인증서 경고는 「고급 → 계속」으로 넘어갑니다).

- `https://www.aurabus.com` → **홈페이지**가 떠야 합니다
- `https://majung.aurabus.com` → **손잡고 마중 로그인 화면**이 떠야 합니다

**여기서 제대로 갈리면 다 된 것입니다.** 남은 것은 바깥에서 들어오는 길뿐입니다.

확인이 끝나면 **그 세 줄을 지우고 저장하세요.** 그대로 두면 나중에 진짜 서비스가
아니라 계속 NAS 를 직접 보게 되어, 밖에서 안 되는 것을 눈치채지 못합니다.
...
| 포트 | 어디로 | 왜 |
|---|---|---|
| `80` | **NAS** | 인증서 발급·갱신에 필요합니다 |
| `443` | **NAS** | 실제 서비스 |
| 그 밖의 모든 것 | **지웁니다** | |

**특히 `8043` 은 반드시 지우세요.** 지금 DSM 관리 화면이 인터넷에 열려 있습니다.
`5000` · `5001` · `9292`(SSH) 도 열려 있으면 함께 지웁니다.

이 순간 맥미니는 더 이상 쓰이지 않습니다. 꺼두셔도 됩니다.

### 5단계 — 인증서를 받는다

6장대로 Let's Encrypt 인증서를 받아 세 주소에 붙입니다. 80 포트가 NAS 로 와야
발급되므로 4단계 다음입니다.

### 6단계 — 밖에서 확인한다

**휴대폰 테더링으로 바꾼 뒤** 내 PC 에서:

```powershell
python tools\live_check.py
```

인증서·화면·`http`→`https` 넘김·숨어야 할 관리 포트를 한꺼번에 봅니다.
...
**`-p 9292` 를 빠뜨리면 안 됩니다.** 이 NAS 는 SSH 를 22번이 아니라 **9292번**에
두었습니다 (기본 포트를 그대로 두면 밖에서 두드리는 시도가 끊이지 않습니다).

처음 들어가면 「계속하시겠습니까」를 묻는데 `yes` 를 칩니다.
...
`Dockerfile`, `docker-compose.yml`, `src`, `deploy` 같은 것들이 보이면 된 것입니다.
...
### 5-6. 첫 유치원 자리를 깔아 드린다

가입 신청 → 승인을 거치지 않고 **우리가 먼저 열어 드리는** 경우입니다.
시범 운영이 그렇습니다. 이 한 줄이면 운영자 계정과 유치원, 그 원의 총괄 관리자가
한꺼번에 생깁니다.
...
무엇이 만들어지나 — 반 6개, 귀가 차수 5단계, 학원 5곳, 차량 1대가 기본값으로
채워집니다. 들어가자마자 빈 화면을 마주하지 않게 하려는 것이고, 설정 화면에서
그 원에 맞게 고치시면 됩니다. **원아는 넣지 않습니다** — 명부를 올리는 것이
그 원의 첫 일입니다.

> **처음 비밀번호는 두 계정 모두 첫 로그인 때 다시 정하게 됩니다.**
> 우리가 정해 건넨 값은 이미 카톡이나 문자를 거쳤습니다. 바꾸기 전까지는
> 어느 화면도 열리지 않습니다.
>
> 그래도 `1234` 같은 짧은 값은 **건네고 나서 그날 안에** 쓰시게 하는 것이 좋습니다.
> 주소가 이미 인터넷에 열려 있다면, 본인이 처음 들어가기 전에 남이 먼저
> 들어갈 수 있습니다. 자리를 깔아 드리는 것과 알려 드리는 것을 같은 날에 하세요.

### 유치원을 더 열 때

같은 명령에서 이름만 바꾸면 됩니다 (`--operator` 는 빼고 — 운영자는 하나입니다).

```bash
docker compose run --rm majung firstout --setup-only \
  --open-kinder "봄뜰유치원" --admin "김선생:kimss:1234"
```

같은 이름의 유치원이나 같은 아이디가 이미 있으면 아무것도 하지 않습니다.
...
| `5000` · `5001` | DSM 관리 화면 |
| `9292` | SSH (이 NAS 는 22번이 아니라 9292번을 씁니다) |
...
처음 누르면 **접속 정보를 한 번만** 물어봅니다. 셋 다 그냥 엔터를 치면
지금 쓰는 값(`aurabus@192.168.100.10` · `/volume1/docker/majung` · 포트 `9292`)이
들어갑니다.
